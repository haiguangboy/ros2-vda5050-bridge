...
class SimpleNavPathPublisher(Node):
    ...
    def generate_and_publish_path(self):
        ...
        for i in range(NUM_PATH_POINTS):
            ...
            # 位置：只有x坐标递增，其他保持相同
            current_x = base_x + i * X_INCREMENT
        ...
        print(f"  Yaw: {start_point['yaw']:.3f} rad")
        ...
        print(f"  Yaw: {end_point['yaw']:.3f} rad")
        ...
        for point in path_points:
            print(f"Point {point['index']}: (x={point['x']:.3f}, y={point['y']:.3f}, yaw={point['yaw']:.3f} rad)")

        print("=" * 60)

        self.get_logger().info(f'Published path with {len(path_msg.poses)} poses. '
                              f'Start: ({start_point["x"]:.2f}, {start_point["y"]:.2f}, {start_point["yaw"]:.3f} rad) '
                              f'End: ({end_point["x"]:.2f}, {end_point["y"]:.2f}, {end_point["yaw"]:.3f} rad)')
